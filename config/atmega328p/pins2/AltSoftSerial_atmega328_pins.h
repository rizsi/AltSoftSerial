...
/** Setup pins for serial: TX to output, RX to input */
	...
	#define SETUP_PINS() \
		...
		PORTB|=0b00000001;	\
		PORTB&=~0b00000100;	\
		DDRB|=0b00000100;	/* OC1B - PB2 to output low */

	/** On output compare match the output is set to low */
	#define CONFIG_MATCH_CLEAR()		(TCCR1A = TCCR1A | ((1<<COM1B1) | (1<<COM1B0)))
	/** On output compare match the output is set to high */
	 #define CONFIG_MATCH_SET()		(TCCR1A = (TCCR1A | (1<<COM1B1)) & ~(1<<COM1B0)) 
	...
	#define SETUP_PINS() \
		...
		PORTB|=0b00000001;	\
		PORTB|=0b00000100;	\
		DDRB|=0b00000100;	/* OC1B - PB2 to output high */
